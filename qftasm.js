...
function updataRAMslots() {
    $($('#ram-bank tr').slice(1)).remove();
    for (i = 0; i < RAM.length; i++) {
        item = RAM[i];
        if (item[0] || item[1] || item[2]) {
            $('#ram-bank').append($('<tr><td>' + i + '</td><td>' + bin(i) + `</td><td>${item[0]}</td><td>` + bin(item[0]) + `</td><td>${item[1]}</td><td>${item[1]}</td></tr>`));
        }
    }
}
...
function RAMwrite(addr, val, inc = 1) {
    ...
    if (RAMdisplay.indexOf(addr) >= 0) {
        update_display(addr);
    }
    if (bps_write.indexOf(addr) >= 0) {
        stop_code();
    }
...
function MNZ(test, val, dest) {
    if (test) {
        RAMwrite(dest, val);
    }
}

function MLZ(test, val, dest) {
    if (test < 0 || test >= 32768) {
        RAMwrite(dest, val);
    }
}

function ADD(val1, val2, dest) {
    RAMwrite(dest, val1 + val2);
}

function SUB(val1, val2, dest) {
    RAMwrite(dest, val1 - val2);
}

function AND(val1, val2, dest) {
    RAMwrite(dest, val1 & val2);
}

function OR(val1, val2, dest) {
    RAMwrite(dest, val1 | val2);
}

function XOR(val1, val2, dest) {
    RAMwrite(dest, val1 ^ val2);
}

function ANT(val1, val2, dest) {
    RAMwrite(dest, val1 & (val2 ^ 65535));
}

function SL(val1, val2, dest) {
    RAMwrite(dest, val1 << val2);
}

function SRL(val1, val2, dest) {
    RAMwrite(dest, (val1 & 65535) >> val2);
}

function SRA(val1, val2, dest) {
    RAMwrite(dest, ((val1 & 65535) >> val2) + ((val1 & 65535) >= 32768 ? ((1 << val2) - 1) << (16 - val2) : 0));
}
...
function set_code() {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        var parts = nocomment.split(/\s+/);

        if (opnames.indexOf(parts[1]) < 0) {
            $('#error').text("Command " + parts[1] + " on line " + i + " is invalid.");
            return;
        }

        new_program.push({
            "line-num": parts[0].slice(0, parts[0].length - 1),
            "opname": parts[1],
